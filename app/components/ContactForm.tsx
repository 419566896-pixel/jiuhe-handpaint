...
export default function ContactForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.ok) {
        setMessage('✅ 提交成功！我们将在48小时内与您联系。');
        setSubmitted(true);
        (e.target as HTMLFormElement).reset();
      } else {
        setMessage(`⚠️ ${result.error || '提交失败，请重试'}`);
      }
    } catch (error) {
    ...
    <>
      {!submitted ? (
        ...
        <div className="text-center py-8">
          ...
          <h3 className="text-xl font-bold text-white mb-2">预约提交成功！</h3>
          <p className="text-gray-400 mb-6">我们将在48小时内通过电话或微信与您联系</p>
          <button
